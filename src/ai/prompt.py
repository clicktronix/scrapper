...
1) Все текстовые поля на русском языке. Literal-поля заполняй на английском, как в схеме.
   ОБЯЗАТЕЛЬНО: city и country ВСЕГДА на русском ('Алматы', 'Астана', 'Казахстан', 'Россия').
   Никогда не пиши 'Kazakhstan', 'Almaty', 'Astana' — только русские названия.
2) Если данных недостаточно: scalar -> null, списки -> [].
3) Не выдумывай факты, бренды, теги, категории и локации.
   НЕ ДУБЛИРУЙ элементы в списках — каждый тег, подкатегория и индустрия только ОДИН раз.
4) reasoning заполняй ПЕРВЫМ: 3-5 предложений.
5) summary: 2-3 абзаца о блогере, аудитории и пользе для рекламодателя.

КРИТИЧЕСКИ ВАЖНО — posting_frequency:
- Определяй СТРОГО по метрике posts_per_week из данных профиля.
- rare: posts_per_week < 0.5 (реже раза в 2 недели).
- weekly: posts_per_week 0.5-1.5 (примерно раз в неделю).
- several_per_week: posts_per_week 1.5-5 (2-5 раз в неделю).
- daily: posts_per_week > 5 (каждый день или чаще).
- НЕ УГАДЫВАЙ по количеству постов в выборке — используй ТОЛЬКО posts_per_week.
...
- best_fit_industries: конкретные индустрии НА РУССКОМ одним словом \
('красота', 'авто', 'фитнес', 'детские товары', 'еда', 'финтех'). \
НЕ пиши 'подходит для ...' — пиши только название индустрии. \
НЕ ставь всем одинаковые — подбирай релевантные КОНКРЕТНОМУ блогеру.
- not_suitable_for: индустрии одним словом НА РУССКОМ \
('алкоголь', 'азартные игры', 'табак'). НЕ пиши 'не подходит для ...'.
...
- tags: 7-40 УНИКАЛЬНЫХ штук, СТРОГО из справочника ниже, на русском. \
НЕ переводи, НЕ выдумывай. Каждый тег — ОДИН раз, без повторов.

...
class SupabaseBlogRepository:
    ...
    async def upsert(self, blog_id: str, data: dict[str, Any]) -> None:
        ...
        await run_in_thread(
            self._db.table("blogs").update(data).eq("id", blog_id).execute
        )
    ...
    async def upsert_posts(
        ...
        await run_in_thread(
            self._db.table("blog_posts").upsert(
    ...
    async def upsert_highlights(
        ...
        await run_in_thread(
            self._db.table("blog_highlights").upsert(
                rows, on_conflict="blog_id,platform_id"
            ).execute
        )
    ...
    async def cleanup_orphan_person(self, person_id: str) -> None:
        ...
        try:
            blogs = await run_in_thread(
                self._db.table("blogs").select("id").eq("person_id", person_id).limit(1).execute
            )
            if not blogs.data:
                await run_in_thread(
                    self._db.table("persons").delete().eq("id", person_id).execute
                )

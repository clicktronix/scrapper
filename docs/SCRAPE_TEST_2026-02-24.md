...
| HikerAPI баланс ПОСЛЕ | $19.15 |
| HikerAPI потрачено | **$1.96** (98 запросов) |
| HikerAPI стоимость на запрос | **$0.02** |
| HikerAPI стоимость на профиль | **$0.098** (~5 запросов/профиль) |
| OpenAI Batch стоимость (всего) | **$0.045** |
| OpenAI Batch стоимость на профиль | **$0.0023** |
| **Итого на профиль** | **~$0.10** |
| **Итого на 20 профилей** | **$2.005** |
...
### Прогноз полной стоимости (HikerAPI + OpenAI)

| Масштаб | HikerAPI (~$0.098) | OpenAI (~$0.0023) | Итого |
|---------|-------------------|-------------------|-------|
| 100 профилей | ~$9.80 | ~$0.23 | **~$10.03** |
| 500 профилей | ~$49.00 | ~$1.13 | **~$50.13** |
| 1,000 профилей | ~$98.00 | ~$2.25 | **~$100.25** |
| 5,000 профилей | ~$490.00 | ~$11.26 | **~$501.26** |

> **98% стоимости — HikerAPI** (скрапинг), OpenAI-анализ практически бесплатный.

> **Наблюдения по OpenAI:**
...
> - Output-тяжёлый pipeline: 95% стоимости OpenAI — за output tokens (reasoning)

> **Наблюдения по HikerAPI:**
> - 98 запросов на 20 профилей = ~4.9 запросов/профиль (profile_info + medias + highlights + stories)
> - $0.02/запрос — фиксированная цена HikerAPI
> - Текущего баланса $19.15 хватит ещё на ~195 профилей

...
Ошибка при выполнении задачи
       ...
       │
       ├── HikerAPIError (404)
       │     └─ pre_filter: filtered_out: not_found (без retry)
       │        full_scrape: mark_task_failed(retry=False)
       │
...
| `retry_stale_batches` | 2 часа | Батчи >4ч → retry |
| `retry_missing_embeddings` | 1 час | Генерация embedding для блогов без вектора |
| `retry_taxonomy_mappings` | 2 часа | Повторный матчинг категорий/тегов для ai_insights |
| `audit_taxonomy_drift` | Daily 05:00 UTC | Аудит расхождений taxonomy prompt ↔ DB |
...
| `pre_filter_log` | Лог отфильтрованных блогеров (upsert by username+reason) | username, reason, followers_count, avg_likes |
...
user_medias_chunk_v1 + user_clips_chunk_v1
  ...
  ├── like_and_view_counts_disabled → пропуск проверки engagement

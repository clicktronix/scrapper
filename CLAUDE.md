...
- OpenAI Batch API (gpt-5-mini, structured outputs)
...
Миграции SQL лежат в соседнем проекте: `../platform/supabase/migrations/`.
Формат имени: `YYYYMMDDHHMMSS_description.sql`. НЕ применять миграции через Supabase MCP `apply_migration` напрямую — создавать файлы миграций в `../platform/supabase/migrations/` и применять через `supabase db push` или CI.
